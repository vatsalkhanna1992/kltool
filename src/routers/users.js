  ...
  try {
    if (req.body.app_name && req.body.app_secret) {
      ...
      if (!app) {
        ...
        if (
          bcrypt.compare(app_secret, app.app_secret) &&
  ...
  try {
    ...
    if (err) {
      ...
      if (err.password) {
        res.status(400).send({
          error:
            "Please enter valid password and password length should be greater than 8.",
        });
        return;
      }
    }
...
router.post("/forgot-password", async (req, res) => {
  ...
  if (!isValid) {
    res.render("forgot-password", {
      ...
      message:
        'No problem! Please click <a href="/register" class="">here</a> to register.',
    });
    return;
  }
  const new_password = Users.generateRandomString(
    12,
  ...
  try {
    ...
    if (updated_user) {
      res.render("forgot-password", {
        message:
          "Your new password has been sent to your email address. Please use that for logging in.",
      });
      return;
    }
  } catch (e) {
